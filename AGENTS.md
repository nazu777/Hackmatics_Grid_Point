...
- **Phase 2 TODO** (`docs/phases.md:22`): Leaflet/Folium map, bubble sizing `∝ daily_orders`, K selector 1-10, metric toggle, constraint panel, auto-fit bounds, <2s render for 1000 nodes. (OptimizationPanel already adds K/metric/capacity/radius UI — map rendering remains)
...
- **Phase 4 TODO** (`docs/phases.md:49`): cost engine already in `optimization.py:245 evaluate_network_layout` (Σd, Σw·d, cost `293`, avg, utilization `331`), baseline modes already done, but UI comparison cards + map overlays (lines, clusters, R_max circles) + export still to polish.
