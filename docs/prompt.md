> **Archival note (Sept 2026)**: this was the Phase 1 kickoff prompt. It is preserved as-is below for provenance; the implementation has since grown well beyond Phase 1 (road routing, live fuel/traffic, Census seeding, JWT auth, warehouse expansion, Mapbox GL UI, 159 tests). For the current system, read `README.md`, `../AGENTS.md`, `schema.md`, `prd.md`, and `phases.md` first.
> **Backlog pointer (ALL SHIPPED Sept 20 2026)**: user items #1–#14 are recorded verbatim in `phases.md` §0 and divided into concurrent Phases A–F in the same file (Person 1 shipped A/D/E, Person 2 shipped B/C/F); `prd.md` §§5.1/5.2/5.5/5.7/5.9–5.11, `schema.md` §§2.8–2.10 + routes contract, and `demo_script.md` carry the matching shipped spec.
